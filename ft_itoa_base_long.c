...
static int	output_len(long value, int base)
{
	int				i;
	unsigned long	u_val;

	i = 0;
	u_val = (unsigned long)value;
	if (value == 0)
		return (1);
	if (value < 0 && base != 10)
	{
...
static void	neg_check(long *value, int base, int *neg_flag, int *len)
{
	if (base == 10 && *value < 0)
	{
		*neg_flag = 1;
...
static char	*fill_loop(long value, int base, int len, int neg_flag)
{
	int		i;
	int		int_min;
	char	*digit;
...
static char	*u_fill_loop(unsigned long value, int base, int len, int neg_flag)
{
	int		i;
	int		int_min;
	char	*digit;
